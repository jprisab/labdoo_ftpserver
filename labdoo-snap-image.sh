#!/bin/bash
#CRONTABBED DAILY: 20 2 * * * /root/labdoo-snap-image.sh 2>&1 | /usr/bin/logger -t labdoo_image_snap 
# This script creates a backup of a Labdoo AMI
#
# Make sure the aws cli tools are installed locally IN THE OLDER VERSION IT WAS DONE by running these commands:
#
#    $ sudo apt-get update
...
instance_id_prod=          # EC2 Instance to backup (can be production or devevelopement)
max_backups=4                                   # Maximum number of backup AMIs kept
...
#userid_dst=                        # AWS user ID of destination profiles (dst2)
...
state=$(/usr/local/bin/aws ec2 describe-images --owners self --profile $profile_src --query 'Images[*].[Name,State]' | grep $new_ami_name | awk '{print $2}')
if [ "$state" != "" ]; then
	...
	/usr/local/bin/aws ec2 create-image --no-reboot --profile $profile_src --instance-id $instance_id_prod --name $new_ami_name --description "Labdoo production AMI as of "$today
	...
	image_id=$(/usr/local/bin/aws ec2 describe-images --owners self --profile $profile_src --query 'Images[*].[Name,ImageId]' | grep $new_ami_name | awk '{print $2}')
	snapshot_ids=($(/usr/local/bin/aws ec2 --profile $profile_src describe-snapshots --query 'Snapshots[?StartTime>=`'$snapshot_date'`]' | grep $image_id | grep --only-matching "snap-[0-9a-z]*"))
	...
	/usr/local/bin/aws ec2 modify-image-attribute --profile $profile_src --image-id $image_id --launch-permission "{\"Add\":[{\"UserId\":\"$userid_dst\"}]}"
	for snap in "${snapshot_ids[@]}"
	do
		echo "Sharing AMI's snapshot $snap with user $userid_dst"
		/usr/local/bin/aws ec2 modify-snapshot-attribute --profile $profile_src --snapshot-id $snap --attribute createVolumePermission --operation-type add --user-ids $userid_dst 
	done

	# Copy the AMI to another region
	echo "Copying AMI $image_id in region $region_src to destination $region_dst"
	/usr/local/bin/aws ec2 copy-image --profile $profile_dst --source-region $region_src --region $region_dst --source-image-id $image_id --name "Backup-"$new_ami_name

	echo "Waiting for AMI copy to complete..."
	# Wait until the AMI has been copied
	while true
	do
		state=$(/usr/local/bin/aws ec2 describe-images --owners self --profile $profile_dst --query 'Images[*].[Name,State]' | grep $new_ami_name | awk '{print $2}')
		if [ "$state" == "available" ]; then
			break
		fi
		sleep 3
...
/usr/local/bin/aws ec2 modify-image-attribute --profile $profile_src --image-id $image_id --launch-permission "{\"Remove\":[{\"UserId\":\"$userid_dst\"}]}" 
for snap in "${snapshot_ids[@]}"
do
	echo "Revoking sharing rights of AMI's snapshot $snap with user $userid_dst"
	/usr/local/bin/aws ec2 modify-snapshot-attribute --profile $profile_src --snapshot-id $snap --attribute createVolumePermission --operation-type remove --user-ids $userid_dst 
done
fi
...
image_id_del=$(/usr/local/bin/aws ec2 describe-images --owners self --profile $profile_src --query 'Images[*].[Name,ImageId]' | grep $del_ami_name | awk '{print $2}')
if [ "$image_id_del" != "" ]; then
	echo "Deleting/deregistering AMI $del_ami_name which is too old to keep..."
	snapshot_ids=($(/usr/local/bin/aws ec2 --profile $profile_src describe-snapshots --query 'Snapshots[?StartTime>=`'$snapshot_date'`]' | grep $image_id_del | grep --only-matching "snap-[0-9a-z]*"))
	/usr/local/bin/aws ec2 deregister-image --profile $profile_src --image-id $image_id_del 
	for snap in "${snapshot_ids[@]}"
	do
		echo "-----------------------------------------------------------------"
		echo "DELETING AMI's snapshot $snap"
		/usr/local/bin/aws ec2 delete-snapshot --profile $profile_src --snapshot-id $snap
	done
	...
	image_id_del=$(/usr/local/bin/aws ec2 describe-images --owners self --profile $profile_dst --query 'Images[*].[Name,ImageId]' | grep $del_ami_name | awk '{print $2}')
	if [ "$image_id_del" != "" ]; then
		echo "Deleting/deregistering AMI $del_ami_name which is too old to keep..."
		unset sanpshot_ids
		snapshot_ids=($(/usr/local/bin/aws ec2 --profile $profile_dst describe-snapshots --query 'Snapshots[?StartTime>=`'$snapshot_date'`]' | grep $image_id_del | grep --only-matching "snap-[0-9a-z]*"))
		/usr/local/bin/aws ec2 deregister-image --profile $profile_dst --image-id $image_id_del 
		for snap in "${snapshot_ids[@]}"
		do
			echo "------------------------------------------------------------"
			echo "DELETING AMI's snapshot in Backup system: $snap with profile $profile_dst"
			/usr/local/bin/aws ec2 delete-snapshot --profile $profile_dst --snapshot-id $snap
